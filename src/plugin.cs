...
namespace DSPTechTreeUX
{
    ...
    internal sealed class TechTreeView : MonoBehaviour
    {
        ...
        private RectTransform _researchButtonRect;
        private UnityEngine.UI.Button _secondaryActionButton;
        private UnityEngine.UI.Text _secondaryActionButtonText;
        private RectTransform _secondaryActionButtonRect;
        private int _secondaryActionMode; // 0 = hidden, 1 = metadata buyout, 2 = sandbox direct unlock
        ...
        private void CreateDetailPanel()
        {
            ...
            _researchButtonRect = buttonObj.GetComponent<RectTransform>();
            _researchButtonRect.anchorMin = new Vector2(0f, 0f);
            _researchButtonRect.anchorMax = new Vector2(1f, 0f);
            _researchButtonRect.pivot = new Vector2(0.5f, 0f);
            _researchButtonRect.offsetMin = new Vector2(18f, 14f);
            _researchButtonRect.offsetMax = new Vector2(-18f, 54f);
            ...
            _researchButtonText = CreateText("Text", _researchButtonRect, 15, UnityEngine.TextAnchor.MiddleCenter);
            ...
            GameObject secondaryObj = CreateUIObject("SecondaryActionButton", _detailPanel);
            _secondaryActionButtonRect = secondaryObj.GetComponent<RectTransform>();
            _secondaryActionButtonRect.anchorMin = new Vector2(0.5f, 0f);
            _secondaryActionButtonRect.anchorMax = new Vector2(1f, 0f);
            _secondaryActionButtonRect.pivot = new Vector2(0.5f, 0f);
            _secondaryActionButtonRect.offsetMin = new Vector2(4f, 14f);
            _secondaryActionButtonRect.offsetMax = new Vector2(-18f, 54f);

            UnityEngine.UI.Image secondaryBg = secondaryObj.AddComponent<UnityEngine.UI.Image>();
            secondaryBg.color = new Color(0.38f, 0.24f, 0.10f, 1f);
            _secondaryActionButton = secondaryObj.AddComponent<UnityEngine.UI.Button>();
            _secondaryActionButton.targetGraphic = secondaryBg;
            _secondaryActionButton.onClick.AddListener(OnSecondaryActionButtonClick);

            _secondaryActionButtonText = CreateText("Text", _secondaryActionButtonRect, 14, UnityEngine.TextAnchor.MiddleCenter);
            Stretch(_secondaryActionButtonText.rectTransform);
            _secondaryActionButtonText.fontStyle = UnityEngine.FontStyle.Bold;
            _secondaryActionButtonText.raycastTarget = false;
        ...
        private Vector2 ClampPagePan(int page, Vector2 pan, float zoom)
        {
            ...
            // The custom detail panel overlays the right side of the graph. Include its
            // live width in the overscroll allowance so far-right vanilla or modded techs
            // can always be panned fully clear of it.
            float detailOcclusion = (_detailPanel != null && _detailPanel.gameObject.activeInHierarchy)
                ? _detailPanel.rect.width
                : 0f;
            float safeRight = PanSafeRight + detailOcclusion;
            ...
            if (filtered && page >= 0 && page < 2 && _pageVisibleBoundsValid[page])
            {
                ...
                float FilterSafeRight = safeRight;
            ...
            float fullMinX = viewportW - graphW - safeRight;
        ...
        private UITechNode GetSelectedVanillaNode()
        {
            if (_tree == null || _tree.nodes == null || _selectedTechId == 0)
                return null;

            UITechNode node;
            return _tree.nodes.TryGetValue(_selectedTechId, out node) ? node : null;
        }

        private void OnSecondaryActionButtonClick()
        {
            UITechNode node = GetSelectedVanillaNode();
            if (node == null)
                return;

            if (_secondaryActionMode == 2)
                node.OnUnlockDirectButton(0);
            else if (_secondaryActionMode == 1)
                node.OnBuyoutButtonClick(0);
        ...
        private void RefreshSecondaryActionButton()
        {
            if (_secondaryActionButton == null ||
                _secondaryActionButtonText == null ||
                _secondaryActionButtonRect == null ||
                _researchButtonRect == null)
                return;

            _secondaryActionMode = 0;

            if (_selectedTechId != 0 && GameMain.history != null)
            {
                TechProto proto = LDB.techs.Select(_selectedTechId);
                bool researching = GameMain.history.currentTech == _selectedTechId;
                bool canEnqueue = GameMain.history.CanEnqueueTech(_selectedTechId);
                bool prereqSufficient = IsTechReadyByMap(_selectedTechId);

                if (proto != null && !researching)
                {
                    if (GameMain.sandboxToolsEnabled && canEnqueue)
                    {
                        _secondaryActionMode = 2;
                        _secondaryActionButtonText.text = "Unlock instantly";
                    }
                    else if (UITechTree.showProperty &&
                             canEnqueue &&
                             prereqSufficient &&
                             !proto.IsHiddenTech &&
                             !proto.IsObsolete)
                    {
                        _secondaryActionMode = 1;
                        _secondaryActionButtonText.text = "Unlock with metadata";
                    }
                }
            }

            bool showSecondary = _secondaryActionMode != 0;
            _secondaryActionButton.gameObject.SetActive(showSecondary);

            if (showSecondary)
            {
                _researchButtonRect.anchorMin = new Vector2(0f, 0f);
                _researchButtonRect.anchorMax = new Vector2(0.5f, 0f);
                _researchButtonRect.offsetMin = new Vector2(18f, 14f);
                _researchButtonRect.offsetMax = new Vector2(-4f, 54f);
            }
            else
            {
                _researchButtonRect.anchorMin = new Vector2(0f, 0f);
